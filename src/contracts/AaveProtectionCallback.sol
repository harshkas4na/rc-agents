...
/**
 * @title AaveProtectionCallback
 * @notice Server-managed Aave liquidation protection system
 * @dev The server (owner) manages configs on behalf of users (protectedUser).
 *      Each config specifies a protectedUser whose Aave position is monitored.
 */
contract AaveProtectionCallback is AbstractCallback, RescuableBase {
     ...
     * @param _protectedUser Address whose Aave position to protect
    ...
    function createProtectionConfig(
        ...
        require(
            _healthFactorThreshold > MIN_HEALTH_FACTOR,
            "Threshold too low"
        );
        require(
            _targetHealthFactor > _healthFactorThreshold,
        ...
        require(
            _validateAssetSupported(_collateralAsset),
            "Collateral asset not supported"
        );
        require(
            _validateAssetSupported(_debtAsset),
            "Debt asset not supported"
        );
    ...
    /**
     * @notice Checks and protects all active configs (called by RC via CRON)
    ...
    function checkAndProtectPositions(
        ...
        for (uint256 i = 0; i < protectionConfigs.length; i++) {
            ...
            try this._checkAndProtectConfig(i) returns (bool wasProtected) {
                ...
                emit ProtectionCheckFailed(
                    i,
                    "Unexpected error during protection check"
                );
            }
        }
    ...
    function _checkAndProtectConfig(uint256 configId) external returns (bool) {
        ...
        // Get current health factor for the protected user
        (, , , , , uint256 currentHealthFactor) = ILendingPool(lendingPool)
            .getUserAccountData(config.protectedUser);
        ...
        if (
            config.lastExecutionAttempt > 0 &&
            block.timestamp < config.lastExecutionAttempt + RETRY_COOLDOWN
        ) {
            return false;
        }
        ...
        if (config.protectionType == ProtectionType.COLLATERAL_DEPOSIT) {
            protectionExecuted = _executeCollateralProtection(
                configId,
                currentHealthFactor
            );
        } else if (config.protectionType == ProtectionType.DEBT_REPAYMENT) {
            protectionExecuted = _executeDebtRepayment(
                configId,
                currentHealthFactor
            );
     ...
     */
    function cancelProtectionConfig(
        uint256 configId
     ...
     */
    function pauseProtectionConfig(
        uint256 configId
    ) external onlyOwner validConfig(configId) {
        ProtectionConfig storage config = protectionConfigs[configId];
        require(
     ...
     */
    function resumeProtectionConfig(
        uint256 configId
    ) external onlyOwner validConfig(configId) {
        ProtectionConfig storage config = protectionConfigs[configId];
        require(
    ...
    function _executeCollateralProtection(
        ...
        try this._performCollateralProtection(configId) returns (
            uint256 collateralAdded
        ) {
            if (collateralAdded > 0) {
                ProtectionConfig storage config = protectionConfigs[configId];
                (, , , , , uint256 finalHealthFactor) = ILendingPool(
                    lendingPool
                ).getUserAccountData(config.protectedUser);
            ...
            emit ProtectionCheckFailed(
                configId,
                string(
                    abi.encodePacked("Collateral protection failed: ", reason)
                )
            );
            return false;
        } catch {
            emit ProtectionCheckFailed(
                configId,
                "Collateral protection failed: Unknown error"
            );
            return false;
        }
    }
    ...
    function _executeDebtRepayment(
        ...
        try this._performDebtRepayment(configId) returns (
            ...
            emit ProtectionCheckFailed(
                configId,
                string(abi.encodePacked("Debt repayment failed: ", reason))
            );
            return false;
        } catch {
            emit ProtectionCheckFailed(
                configId,
                "Debt repayment failed: Unknown error"
            );
            return false;
        }
    }
    ...
    function _performCollateralProtection(
        ...
        (, uint256 totalDebtUSD, , , , ) = ILendingPool(lendingPool)
            .getUserAccountData(config.protectedUser);
        if (totalDebtUSD == 0) {
            return 0;
        }
        ...
        if (collateralNeeded > 0) {
            uint256 userBalance = IERC20(config.collateralAsset).balanceOf(
                config.protectedUser
            );
            require(
                userBalance >= collateralNeeded,
                "Insufficient user balance for collateral"
            );

            uint256 approvedAmount = IERC20(config.collateralAsset).allowance(
                config.protectedUser,
                address(this)
            );
            require(
                approvedAmount >= collateralNeeded,
                "Insufficient approved collateral"
            );

            IERC20(config.collateralAsset).safeTransferFrom(
                config.protectedUser,
                address(this),
                collateralNeeded
            );
            IERC20(config.collateralAsset).forceApprove(
                lendingPool,
                collateralNeeded
            );
    ...
    function _performDebtRepayment(
        ...
        (, , uint256 currentVariableDebt, , , , , , ) = IProtocolDataProvider(
            protocolDataProvider
        ).getUserReserveData(config.debtAsset, config.protectedUser);
        ...
        if (repaymentAmount > 0) {
            uint256 userBalance = IERC20(config.debtAsset).balanceOf(config.protectedUser);
            require(
                userBalance >= repaymentAmount,
                "Insufficient user balance for repayment"
            );

            uint256 approvedAmount = IERC20(config.debtAsset).allowance(
                config.protectedUser,
                address(this)
            );
            require(
                approvedAmount >= repaymentAmount,
                "Insufficient approved debt asset"
            );

            IERC20(config.debtAsset).safeTransferFrom(
                config.protectedUser,
                address(this),
                repaymentAmount
            );
    ...
    function _calculateCollateralNeeded(
        ...
        (
            ...
            ,
            uint256 currentHealthFactor
        ) = ILendingPool(lendingPool).getUserAccountData(config.protectedUser);

        if (
            totalDebtUSD == 0 ||
            currentHealthFactor >= config.targetHealthFactor
        ) {
            return 0;
        }

        uint256 collateralLiquidationThreshold = _getAssetLiquidationThreshold(
            config.collateralAsset
        );
        uint256 currentWeightedCollateral = (totalCollateralUSD *
            currentLiquidationThreshold) / 10000;
        uint256 targetHF_BasisPoints = config.targetHealthFactor / 1e14;
        uint256 requiredWeightedCollateral = (targetHF_BasisPoints *
            totalDebtUSD) / 10000;
        ...
        uint256 additionalWeightedCollateral = requiredWeightedCollateral -
            currentWeightedCollateral;
        uint256 additionalCollateralUSD = (additionalWeightedCollateral *
            10000) / collateralLiquidationThreshold;

        uint256 collateralPriceUSD = _getAssetPrice(config.collateralAsset);
        require(
            collateralPriceUSD > 0,
            "Invalid collateral price from Aave oracle"
        );

        uint256 collateralNeeded = (additionalCollateralUSD * 1e18) /
            collateralPriceUSD;
    ...
    function _calculateRepaymentAmount(
        ...
        uint256 weightedCollateral = (totalCollateralUSD *
            currentLiquidationThreshold) / 10000;
        uint256 targetDebtUSD = (weightedCollateral * 10000) /
            (config.targetHealthFactor / 1e14);
        ...
        require(
            debtAssetPriceUSD > 0,
            "Invalid debt asset price from Aave oracle"
        );

        uint8 decimals = IERC20Detailed(config.debtAsset).decimals();
        uint256 assetDebtUSD = (debtAssetPriceUSD * currentVariableDebt) /
            (10 ** decimals);
        ...
        if (assetDebtUSD <= debtToRepayUSD) {
            ...
            tokensToRepay =
                (debtToRepayUSD * currentVariableDebt) /
                assetDebtUSD;
        }
     ...
     */
    function _getAssetLiquidationThreshold(
        address asset
    ) internal view returns (uint256) {
        (
            ,
     ...
     */
    function _validateAssetSupported(
        address asset
    ...
    /**
     * @notice Get user protection details
     * @param user Address to get protection details for
     */
    function getUserProtection(
        address user
    )
        external
        view
    ...
    {
        // Return latest active config for the given user
        for (uint256 i = protectionConfigs.length; i > 0; i--) {
            ProtectionConfig storage config = protectionConfigs[i - 1];
            if (config.status == ProtectionStatus.Active && config.protectedUser == user) {
                return (
                    true,
        ...
        return (
            false,
            ProtectionType.COLLATERAL_DEPOSIT,
            0,
            0,
            address(0),
            address(0),
