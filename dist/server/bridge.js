...
exports.fundRCGasPool = fundRCGasPool;
 ...
 */
const GAS_PER_CALLBACK = 100000n;
...
const SWAP_ROUTER_ABI = (0, viem_1.parseAbi)([
 ...
 */
async function fundRCGasPool(usdcAmount, durationSeconds) {
    const breakdown = await computeBreakdown(usdcAmount, durationSeconds);
    console.log(`[bridge] ── Funding breakdown ──`);
    console.log(`[bridge]   Payment:      ${(0, viem_1.formatUnits)(breakdown.totalUsdc, 6)} USDC`);
    console.log(`[bridge]   Margin (20%): ${(0, viem_1.formatUnits)(breakdown.serverMargin, 6)} USDC (kept)`);
    console.log(`[bridge]   Swap (80%):   ${(0, viem_1.formatUnits)(breakdown.swapAmount, 6)} USDC → ETH`);
    ...
    try {
        ...
        console.log(`[bridge]   ETH received: ${(0, viem_1.formatEther)(swapResult.ethReceived)}`);
        ...
        if (bridgeAmount > 0n) {
            try {
                const rcAddr = (process.env.AAVE_PROTECTION_REACTIVE_ADDRESS ?? "");
 ...
 */
async function computeBreakdown(usdcAmount, durationSeconds) {
    const swapAmount = (usdcAmount * SWAP_ALLOCATION_BPS) / 10000n;
    const serverMargin = usdcAmount - swapAmount;
    // Query live Lasna gas price
    let gasPrice;
    try {
        gasPrice = await chain_1.lasnaClient.getGasPrice();
    }
    catch {
    ...
    const BASE_GAS_RESERVE = 1000000000000000n; // 0.001 ETH
    ...
    const ethEstimateWei = (swapAmount * 10n ** 18n) / 2500000000n; // rough @ $2500/ETH
    ...
    console.log(`[bridge]   lREACT needed: ${(0, viem_1.formatEther)(lreactNeededWei)} lREACT`);
    console.log(`[bridge]   ETH to bridge (for RC gas): ${(0, viem_1.formatEther)(rcGasWei)} ETH`);
    return {
        totalUsdc: usdcAmount,
        serverMargin,
        swapAmount,
        rcGasWei,
        gasReserveEth: (0, viem_1.formatEther)(gasReserveWei),
        bridgeAmountEth: (0, viem_1.formatEther)(bridgeWei),
    };
}
// ── Uniswap V3 swap ──────────────────────────────────────────────────────────
async function getClients() {
    ...
    const account = (0, accounts_1.privateKeyToAccount)(hex);
    const rpc = process.env.BASE_SEPOLIA_RPC_URL ?? "https://sepolia.base.org";
    return {
        account,
        publicClient: (0, viem_1.createPublicClient)({ chain: chains_1.baseSepolia, transport: (0, viem_1.http)(rpc) }),
        walletClient: (0, viem_1.createWalletClient)({ account, chain: chains_1.baseSepolia, transport: (0, viem_1.http)(rpc) }),
    };
}
/**
 ...
 */
async function swapUsdcToEth(usdcAmount) {
    ...
    return { txHash: swapTx, ethReceived: wethBalance };
}
...
const LASNA_BRIDGE_ADDRESS = "0x2afaFD298b23b62760711756088F75B7409f5967";
const BRIDGE_ABI = (0, viem_1.parseAbi)([
    "function request(address recipient) external payable",
]);
const WETH_WITHDRAW_ABI = (0, viem_1.parseAbi)([
 ...
 */
async function bridgeEthToLasna(ethAmount, rcAddress) {
    if (ethAmount > 5n * 10n ** 18n) {
        throw new Error(`Bridge amount ${(0, viem_1.formatEther)(ethAmount)} ETH exceeds the 5 ETH per-tx limit. Split into multiple calls.`);
    }
    const { publicClient, walletClient } = await getClients();
    // 1. Unwrap WETH → native ETH
    console.log(`[bridge]   Unwrapping ${(0, viem_1.formatEther)(ethAmount)} WETH → ETH...`);
    ...
    console.log(`[bridge]   Bridging ${(0, viem_1.formatEther)(ethAmount)} ETH → lREACT to RC at ${rcAddress}...`);
